...
func GetSite(w http.ResponseWriter, r *http.Request, proute routes.Proute) {
	...
	q += `	)`
	q += `	 FROM (SELECT si.code, si.name, si.city_name, si.city_geonameid, si.centroid, si.occupation, si.altitude, si.created_at, si.updated_at, d.name as database_name, si.start_date1, si.start_date2, si.end_date1, si.end_date2, (SELECT array_to_json(array_agg(row_to_json(d_src))) FROM (SELECT firstname || ' ' || lastname as fullname FROM "user" u LEFT JOIN database__authors da ON u.id = da.user_id WHERE da.database_id = si.database_id) d_src) as authors FROM site si LEFT JOIN database d ON si.database_id = d.id WHERE si.id = s.id) site_infos`
	q += `)`
	q += `|| '}}'`
